...
assert.match(mapDataHook, /if \(date\) params\.set\('date', date\)/);
assert.match(mapDataHook, /else if \(period\) params\.set\('period', period\)/);
assert.match(mapDataHook, /if \(barangay\) params\.set\('barangay', barangay\)/);
assert.match(mapDataHook, /const incidentQuery = params\.size > 0 \? `\?\$\{params\.toString\(\)\}` : ''/);
assert.match(mapIncidents, /MapPeriodSchema/);
assert.match(mapIncidents, /manilaOperationalPeriodBounds/);
