...
export function VerificationDetails({ request, onOverrideClassification, onUpdateCoordination, onConfirmRelatedReport, onKeepRelatedReportSeparate, isProcessing }: VerificationDetailsProps) {
  ...
  return <main className="flex-1 overflow-y-auto bg-slate-50 p-5 lg:p-7"><section className="mx-auto max-w-6xl space-y-5">
    <header className="border-b border-slate-200 pb-5">
      <div className="flex flex-wrap items-center gap-2"><Badge className={cn("border-0 px-3 py-1.5 text-xs font-black uppercase", severityClass(request.severity))}>{request.severity} severity</Badge><Badge variant="outline" className="border-blue-100 bg-blue-50 font-mono text-sm font-bold text-[#1E3A8A]">{request.requestId}</Badge></div>
      <div className="mt-3 flex flex-wrap items-start justify-between gap-4"><div><h1 className="text-3xl font-black tracking-tight text-[#123B82] lg:text-4xl">{request.type}</h1><p className="mt-1.5 text-base text-slate-500">Reported by <span className="font-semibold text-slate-700">{request.resident.fullName}</span> - {request.reporterType === "GUEST" ? "Guest Mode" : "Registered account"}</p></div><div className="flex flex-wrap gap-x-5 gap-y-2 text-base text-slate-600"><span className="flex items-center gap-1.5"><MapPin className="size-5 text-rose-600" />{request.location}</span><span className="font-medium">Received {formatDistanceToNow(new Date(request.receivedAt), { addSuffix: true })}</span></div></div>
    </header>

    <div role="status" className={cn("flex gap-3 rounded-lg border px-5 py-4", support.className)}><SupportIcon className="mt-0.5 size-7 shrink-0" /><div><p className="text-lg font-bold">{support.title}</p><p className="mt-0.5 text-base opacity-80">{support.detail}</p></div></div>

    <div className="grid gap-4 sm:grid-cols-2"><Metric icon={<Users className="size-6" />} label="People involved" value={String(request.peopleInvolved)} /><Metric icon={<AlertTriangle className="size-6" />} label="Priority" value={request.severity} color={severityClass(request.severity)} /><Card className="border-slate-200 bg-white p-5 shadow-none"><p className="text-sm font-semibold text-slate-500">Incident type</p><p className="mt-1 text-xl font-black text-[#123B82]">{request.type}</p><Badge variant="outline" className="mt-2 text-xs">{request.nature}</Badge></Card><Card className="border-slate-200 bg-white p-5 shadow-none"><p className="text-sm font-semibold text-slate-500">Workflow status</p><p className="mt-1 text-xl font-black text-[#123B82]">{workflow}</p><p className="mt-2 text-sm text-slate-500">{isAwaitingResponder ? "The offer remains in the background; you can continue reviewing other reports." : "Status is updated from the server."}</p></Card></div>

    <div className="grid gap-4 lg:grid-cols-[1.35fr_.85fr]"><div className="relative min-h-64 overflow-hidden rounded-xl border border-slate-200 bg-slate-100 shadow-sm">{request.imageUrl ? <img src={request.imageUrl} alt="User-submitted incident evidence" className="h-full min-h-64 w-full object-cover" /> : <div className="flex min-h-64 flex-col items-center justify-center gap-2 text-base text-slate-500"><Info className="size-8 opacity-40" />No evidence photo submitted</div>}<Badge className="absolute left-3 top-3 border-0 bg-[#1E3A8A] px-2.5 py-1 text-sm font-bold">User submitted</Badge></div><Card className="flex min-h-64 flex-col justify-between border-slate-200 bg-white p-5 shadow-none"><div><p className="text-sm font-black uppercase tracking-wide text-[#1E3A8A]">Location review</p><MapPin className="mt-5 size-9 text-rose-600" /><p className="mt-3 text-lg font-bold text-slate-900">{request.location}</p><p className="mt-1 text-base text-slate-500">Use the Command Map for the report pin and active response context.</p></div>{request.photoLatitude !== undefined && request.photoLongitude !== undefined ? <p className="mt-4 text-sm text-slate-500">Evidence GPS: {request.photoLatitude.toFixed(5)}, {request.photoLongitude.toFixed(5)}</p> : null}</Card></div>
...
function Metric({ icon, label, value, color }: { icon: ReactNode; label: string; value: string; color?: string }) {
  return <Card className="border-slate-200 bg-white p-5 shadow-none"><div className="flex items-center gap-3"><span className={cn("grid size-11 place-items-center rounded-full bg-blue-50 text-[#1E3A8A]", color && `${color} text-white`)}>{icon}</span><div><p className="text-sm font-semibold text-slate-500">{label}</p><p className="text-2xl font-black text-slate-900">{value}</p></div></div></Card>
}
