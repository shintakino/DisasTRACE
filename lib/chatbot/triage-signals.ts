...
const ROUTINE_NON_EMERGENCY_PATTERN = /\b(?:non[- ]?emergency|other\s+request|general\s+assistance|medical\s+advice|checkup|routine\s+(?:consultation|transport)|stable\s+(?:patient|pasyente)|lagnat|nilalagnat|nahihilo|nanlalabo(?:\s+ang)?\s+(?:mata|paningin)|nahihirapan\s+(?:(?:ako|siya)ng\s+)?lunukin|hindi\s+makalunok|hindi\s+makakain|masakit\s+(?:ang\s+)?(?:tiyan|ulo)|pagtatae|nagsusuka|namamaga\s+(?:ang\s+)?(?:kamay|paa|mata)|pantal|nangangati|panic|nagpapanic|naipit\s+(?:ang\s+)?(?:kamay|daliri)\s+sa\s+pinto|(?:na)?gasgas|minor\s+(?:injury|wound)|sprain|pilay|hindi\s+maigalaw\s+(?:ang\s+)?(?:tuhod|balikat)|nasugatan\s+sa\s+(?:baso|kutsilyo)|nasunog\s+(?:ang\s+)?balat)\b/i;
